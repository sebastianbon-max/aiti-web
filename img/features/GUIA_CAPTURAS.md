...
| 8 | IPAM | **Dashboard:** KPIs arriba (total subredes, IPs asignadas, libres, % ocupación) + tabla de subredes con barras de progreso de ocupación en colores (verde/amarillo/rojo). Botones "Ciclo de Vida" y "Conflictos" visibles. | **Mapa de IPs:** detalle de una subred /24 mostrando la grilla de 254 IPs con colores: azul=asignada, verde=libre, naranja=reservada, borde naranja=DHCP. Botones "Próxima IP Libre" y "Verificar IP" visibles. | **Historial + Conflictos:** la vista de historial completo mostrando eventos (badges: Asignada verde, Liberada rojo, Reasignada naranja) con timestamps, o la lista de conflictos detectados con MAC/hostname. | `redes-1.webp` a `redes-8.webp` (ya subidos) |
...
| 15 | Credenciales | **Listado por nivel:** tabla de credenciales SSH organizadas por tipo, marca y modelo (herencia en cascada). Que se vean las 3 secciones con usuario y puerto por cada nivel. | **Formulario de creación/edición:** formulario abierto mostrando campos: nivel (tipo/marca/modelo), usuario, contraseña (oculta), puerto, y selector del catálogo correspondiente. | **Test de conexión o detalle de equipo:** la vista de "Probar" credencial con resultado (OK/error), o el detalle de un equipo mostrando qué credencial hereda y de qué nivel. | `credenciales-1.webp`, `credenciales-2.webp`, `credenciales-3.webp` |
...
| 22 | Firmware | **Versiones con recomendadas:** listado de versiones de firmware/SO por modelo con fecha, archivo adjunto, y badge "recomendada". Acceder desde `/firmware/`. | **Reporte de actualizaciones:** vista `/firmware/reporte/` que muestra equipos con firmware distinto al recomendado, agrupados por modelo. Export PDF/Excel. | **Automatizaciones:** listado de comandos programados `/automatizaciones/` con equipo, comando, frecuencia, último resultado y próxima ejecución. | `firmware-1.webp`, `firmware-2.webp`, `firmware-3.webp` |
